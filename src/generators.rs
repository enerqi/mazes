    ...
    for cell_coord in grid.iter() {
        ...
        if !neighbours.is_empty() {
            ...
            let link_coord = match neighbours_count {
                1 => neighbours[0],
                _ => neighbours[rng.random_range(..neighbours_count)],
            };
    ...
    Iters: GridIterators<SquareCell>,
{
    ...
    let runs_are_horizontal = rng.random::<bool>();
    ...
    for coordinates_line in batch_iter {
        ...
        for coord in &coordinates_line {
            ...
            let should_close_out = at_run_end_boundary || (!at_close_out_direction_boundary && rng.random::<bool>()); // coin flip

            if should_close_out {
                let sample = rng.random_range(..run.len());
                let run_member = run[sample];

                let close_out_dir = grid.neighbour_at_direction(*run_member, run_close_out_direction);
...
/// Todo: handle masks that have walled off unreachable areas, making some unmasked cells unvisitable
///       and causing the algorithm to run forever.
pub fn wilson<GridIndexType, CellT, Iters>(grid: &mut Grid<GridIndexType, CellT, Iters>, mask: Option<&BinaryMask2D>)
where
    GridIndexType: IndexType,
    CellT: Cell,
    Iters: GridIterators<CellT>,
{
    ...
    let mut cells_on_random_walk: FnvHashSet<CellT::Coord> = utils::fnv_hashset(cmp::max(row_len, col_len) * 4);
    ...
    while visited_count < unmasked_count {
        ...
        let walk_start_cell = random_unvisited_unmasked_cell(
            grid,
    ...
    Iters: GridIterators<CellT>,
{
    ...
    let is_any_neighbour_visited = |cell, visited_set: &BitSet, grid: &Grid<GridIndexType, CellT, Iters>| -> bool {
    ...
    let visited_neighbours = |cell: CellT::Coord,
     ...
     -> Option<CellT::CoordinateSmallVec> {
        let vn: CellT::CoordinateSmallVec = grid
            .neighbours(cell)
            .iter()
            .cloned()
    ...
    let are_all_neighbours_visited_or_masked =
        |cell, visited_set: &BitSet, grid: &Grid<GridIndexType, CellT, Iters>, mask: Option<&BinaryMask2D>| -> bool {
            if let Some(m) = mask {
                grid.neighbours(cell)
                    .iter()
    ...
    while visited_count < unmasked_count {
        ...
        if let Some(new_cell) = next_cell {
            if !is_cell_in_visited_set(new_cell, &visited_cells, grid) {
                ...
                let (hunted_cell, hunteds_visited_neighbours): (CellT::Coord, CellT::CoordinateSmallVec) = grid
                    .iter()
                    .skip_while(|cell| {
                        is_cell_in_visited_set(*cell, &visited_cells, grid)
                            || mask.is_some_and(|m| m.is_masked(*cell))
                    ...
                    .fold(None, |_, cell| {
                        Some((
                            cell,
                            visited_neighbours(cell, &visited_cells, grid)
                ...
                let random_visited_neighbour =
                    hunteds_visited_neighbours[rng.random_range(..hunteds_visited_neighbours.len())];
     ...
     -> Option<CellT::CoordinateSmallVec> {
        let vn: CellT::CoordinateSmallVec = if let Some(m) = mask {
            grid.neighbours(cell)
                .iter()
                .cloned()
                .filter(|c: &CellT::Coord| !is_cell_in_visited_set(*c, visited_set, grid) && !m.is_masked(*c))
                .collect()
        } else {
            grid.neighbours(cell)
                .iter()
    ...
    while !dfs_stack.is_empty() {
        ...
        if let Some(unvisited) = unvisited_neighbours_opt {
            ...
            let next_cell = match unvisited_count {
                1 => unvisited[0],
                _ => unvisited[rng.random_range(..unvisited_count)],
            };
...
pub fn rebuild_random_walls<GridIndexType, CellT, Iters>(
    grid: &mut Grid<GridIndexType, CellT, Iters>,
    wall_count: usize,
) where
    GridIndexType: IndexType,
    ...
    Iters: GridIterators<CellT>,
{
    let max_rebuildable_cells = grid
        .iter()
    ...
    while cells_with_wall_rebuilt.len() < build_target_count {
        let cell_coord =
            random_cell(grid, None, &mut rng).expect("Should always get a random cell if not using a Mask");
        if !cells_with_wall_rebuilt.contains(&cell_coord) {
            let adjacent_linked_cells = grid
                .links(cell_coord)
            ...
            if adjacents_count > 0 {
                let linked: CellT::Coord = adjacent_linked_cells[rng.random_range(..adjacents_count)];
...
#[inline]
fn random_neighbour<GridIndexType, CellT, Iters>(
    cell: CellT::Coord,
    grid: &Grid<GridIndexType, CellT, Iters>,
    rng: &mut SmallRng,
) -> Option<CellT::Coord>
where
    ...
    Iters: GridIterators<CellT>,
{
    grid.neighbour_at_direction(cell, CellT::rand_direction(rng, grid.dimensions(), cell))
}

fn random_cell<GridIndexType, CellT, Iters>(
    grid: &Grid<GridIndexType, CellT, Iters>,
    ...
    Iters: GridIterators<CellT>,
{
    if let Some(mask_and_count) = mask_with_unmasked_count {
        random_unmasked_cell(grid, mask_and_count, rng)
    } else {
        Some(grid.random_cell(rng))
    }
}

#[inline]
fn bit_index<GridIndexType, CellT, Iters>(cell: CellT::Coord, grid: &Grid<GridIndexType, CellT, Iters>) -> usize
where
    GridIndexType: IndexType,
    CellT: Cell,
    Iters: GridIterators<CellT>,
...
#[inline]
fn is_cell_in_visited_set<GridIndexType, CellT, Iters>(
    cell: CellT::Coord,
    visited_set: &BitSet,
    grid: &Grid<GridIndexType, CellT, Iters>,
) -> bool
...
fn visit_cell<GridIndexType, CellT, Iters>(
    cell: CellT::Coord,
...
fn random_unvisited_cell<GridIndexType, CellT, Iters>(
    grid: &Grid<GridIndexType, CellT, Iters>,
    ...
    Iters: GridIterators<CellT>,
{
    ...
    if remaining_unvisited_count > 0 {
        let n = rng.random_range(..remaining_unvisited_count);
...
fn random_unmasked_cell<GridIndexType, CellT, Iters>(
    grid: &Grid<GridIndexType, CellT, Iters>,
    ...
    Iters: GridIterators<CellT>,
{
    ...
    if unmasked_cells != 0 {
        let n = rng.random_range(..unmasked_cells);
        let cells_count = grid.size();
        let cell_index = (0..cells_count)
            .filter(|i| {
...
fn random_unvisited_unmasked_cell<GridIndexType, CellT, Iters>(
    grid: &Grid<GridIndexType, CellT, Iters>,
    ...
    Iters: GridIterators<CellT>,
{
    match (visited_set_with_count, mask_with_unmasked_count) {
        (None, None) => Some(grid.random_cell(rng)),

        (None, Some(mask_and_count)) => random_unmasked_cell(grid, mask_and_count, rng),

        (Some(set_and_count), None) => random_unvisited_cell(grid, set_and_count, rng),
        ...
        (Some((visited, visited_count)), Some((mask, unmasked_count))) => {
            ...
            if remaining_cells != 0 {
                let n = rng.random_range(..remaining_cells);
                let cell_index = (0..cells_count)
                    .filter(|i| {
                        let coord = CellT::Coord::from_row_major_index(*i, grid.dimensions());
...
fn random_unmasked_neighbour<GridIndexType, CellT, Iters>(
    cell: CellT::Coord,
    grid: &Grid<GridIndexType, CellT, Iters>,
    mask: &BinaryMask2D,
    rng: &mut SmallRng,
...
where
    ...
    Iters: GridIterators<CellT>,
{
    let unmasked_neighbours: CellT::CoordinateSmallVec = grid
        .neighbours(cell)
        .iter()
        .cloned()
    ...
    if !unmasked_neighbours.is_empty() {
        ...
        let neighbour_cell = match count {
            ...
            _ => unmasked_neighbours[rng.random_range(..count)],
        };
        Some(neighbour_cell)
    } else {
        None
...
fn unmasked_cells_count<GridIndexType, CellT, Iters>(
    grid: &Grid<GridIndexType, CellT, Iters>,
    mask: Option<&BinaryMask2D>,
) -> usize
where
...
{
    if let Some(m) = mask {
        m.count_unmasked_within_dimensions(
            Width(grid.row_length().expect("invalid rowlength").0),
